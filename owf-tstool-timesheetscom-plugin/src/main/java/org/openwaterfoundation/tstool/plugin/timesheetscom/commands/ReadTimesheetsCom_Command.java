 ...
 */
private HashMap<String,Object> createReadProperties ( boolean debug ) {
...
public boolean editCommand ( JFrame parent ) {
	...
	return (new ReadTimesheetsCom_JDialog ( parent, this )).ok();
}
...
throws InvalidCommandParameterException, CommandWarningException, CommandException {
	...
	try {
		...
		if ( dataStore == null ) {
			...
			if ( (CustomerName != null) && !CustomerName.isEmpty() ) {
				// Have single customer name so try to read the matching time series.
				...
				TS ts = null;
				try {
					HashMap<String,Object> readProperties = createReadProperties ( debug );
